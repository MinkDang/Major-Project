...
function base_price_load() {
	if (quantity_price != 0.0 || quantity_activation) {
		...
		if (original_price < 0) {
			// Temporary mitigation for floating problem. Find more here https://floating-point-gui.de/
			original_price = 0;
		}
		price_load(original_price);
	}
}

// Update price, quantity available
function cal_and_display() {
	quantity_price = original_price * quantity_choice;
...
// ********************* Variables
// Numbers
original_price = 0; // Price before multiplication of quantity NOT PRICE OF BASE
quantity_price = 0; // Price after multiplication of quantity
...
const base_prices = {
	...
	simply_cheese: 5.5,
	margherita: 4
}; // List of objects -- Base: Price
const toppings_prices = {
	anchovies: 2,
	jalapenos: 1.8,
	olives: 2,
	parmesan: 1.3
}; // List of objects -- Topping: Price
...
const hide_show = ["hide_1", "hide_2", "hide_3", "hide_4"];
...
function select_random() {
	let base_keys = Object.keys(base_prices);
	let toppings_keys = Object.keys(toppings_prices);

	let random = Math.floor(Math.random() * base_keys.length);
	let random_key = base_keys[random];

	select_base(document.querySelector("[for=" + CSS.escape(random_key) + "]"));
	...
	for (let i = 0; i < random_key.length; i++) {
		select_toppings(document.querySelector("[for=" + CSS.escape(random_key[i]) + "]"));
	}
}
...
function select_base(el) {
	...
	original_price -= prev_price; // Deduct PREVIOUS price
	original_price += base_prices[el.getAttribute("for")]; // Add CURRENT price
...
function select_toppings(el) {
	...
	if (toppings_choice.includes(el.getAttribute("for"))) {
		...
		// Strange behaviour
		// "checked" is supposed to be false because the toppings is being REMOVED
		...
		original_price -= toppings_prices[el.getAttribute("for")];
		...
		// Strange behaviour
		// "checked" is supposed to be true because the toppings is being ADDED
		document.getElementById(el.getAttribute("for")).checked = true;
